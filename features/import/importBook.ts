...
export async function upgradeLegacyFb2(book: BookRecord): Promise<BookRecord> {
  ...
  const parsed = parseFb2(await decodeFb2(book.file));
...
export async function recoverBookCover(book: BookRecord) {
  if (book.format === 'epub') return (await readEpubMetadata(book.file)).cover;
  return parseFb2(await decodeFb2(book.file)).cover;
}
...
export async function importBook(file: File): Promise<BookRecord> {
  ...
  if (extension === 'epub') {
    ...
    return { id: crypto.randomUUID(), hash, format: 'epub', file, ...metadata, dateAdded: now, progress: 0, updatedAt: now };
  }
  const parsed = parseFb2(await decodeFb2(file));
  return { id: crypto.randomUUID(), hash, format: 'fb2', file, title: parsed.title, authors: parsed.authors, cover: parsed.cover, toc: parsed.toc, fb2Html: parsed.html, fb2Images: parsed.images, dateAdded: now, progress: 0, updatedAt: now };
}
