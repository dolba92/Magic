...
> = {
  light: { label: 'Светлая', bg: '#ffffff', text: '#25201d' },
  cream: { label: 'Кремовая', bg: '#fffaf0', text: '#3c2b22' },
  sepia: { label: 'Сепия', bg: '#f1dfbd', text: '#49301f' },
  gray: { label: 'Серая', bg: '#e7e9eb', text: '#25292c' },
  dark: { label: 'Тёмная', bg: '#292522', text: '#eee7da' },
  black: { label: 'Чёрная', bg: '#080808', text: '#efefef' },
  custom: { label: 'Своя', bg: '#fffaf0', text: '#3c2b22' },
};

const fonts = [
  ['Literata', 'С засечками'],
  ['PT Serif', 'С засечками'],
  ['Merriweather', 'С засечками'],
  ['Source Serif 4', 'С засечками'],
  ['Bitter', 'С засечками'],
  ['Lora', 'С засечками'],
  ['Lora Hand', 'Книжно-рукописный'],
  ['Noto Serif', 'С засечками'],
  ['Georgia', 'С засечками'],
  ['Manrope', 'Без засечек'],
  ['Open Sans', 'Без засечек'],
  ['Verdana', 'Без засечек'],
  ['Arial', 'Без засечек'],
] as const;
...
function readerColors(settings: ReaderSettings) {
  return settings.theme === 'custom'
    ...
    : themes[settings.theme] || themes.cream;
}
              ...
              onChange={(event) =>
                patch({
                  alignment: event.target
                    .value as ReaderSettings['alignment'],
                })
              }
            >
      ...
      <details>
        ...
        <div className="settings-group">
          ...
          <SettingRange
            ...
            max={56}
            suffix=" px"
            onChange={(textIndent) => patch({ textIndent })}
          />
        </div>
      </details>
      ...
      <details>
        ...
        <div className="settings-group">
          <div className="setting-row">
            ...
            <div className="segmented column-choice">
              {([1, 2] as const).map((columns) => (
                <button
                  className={
                    (value.pageColumns || 1) === columns
                      ? 'selected'
                      : ''
                  }
                  key={columns}
                  onClick={() =>
                    patch({ pageColumns: columns })
                  }
                >
                  {columns === 1
                    ? '1 колонка'
            ...
            <small className="setting-note">
              На узком экране всегда используется одна колонка
            </small>
          </div>

          <SettingRange
            label="Ширина области чтения"
        ...
        <div className="settings-group">
          <div className="segmented">
            {(
              ...
              <button
                className={
                  value.imageScale === key
                    ? 'selected'
                    : ''
                }
                key={key}
                onClick={() =>
                  patch({ imageScale: key })
                }
              >
                {label}
              </button>
...
function flattenToc(
  items: TocItem[],
  depth = 0,
): Array<{ item: TocItem; depth: number }> {
...
function hasDocumentSelection(documentRef: Document) {
  try {
...
function epubRules(settings: ReaderSettings) {
  ...
  const imageHeight =
    settings.imageScale === 'compact'
      ? 62
      : settings.imageScale === 'large'
        ? 100
        : 82;
  ...
  return {
    ...
    'html body': {
      ...
      'text-align': `${settings.alignment} !important`,
    ...
    'html body p': {
      ...
      'text-indent': `${settings.textIndent}px !important`,
      'line-height': `${settings.lineHeight} !important`,
      orphans: '2',
      widows: '2',
    },

    'html body li, html body blockquote': {
      'line-height': `${settings.lineHeight} !important`,
    },
    ...
    'html body img, html body svg, html body video, html body canvas':
      {
        ...
        'max-width': '100% !important',
        'max-height': `min(${imageHeight}vh, max(80px, calc(100vh - ${verticalSpace}px))) !important`,
...
function applyEpubSettings(
  ...
  rendition.themes.default(epubRules(settings));
...
function applyEpubContent(
  ...
  for (const [property, value] of Object.entries(
    ...
    contents.css(property, value, true);
  }
}

function applyAllEpubContent(
  rendition: any,
  settings: ReaderSettings,
) {
  const current = rendition.getContents() as any;

  for (const contents of Array.isArray(current)
    ? current
    : [current]) {
    if (contents) {
...
function epubSpread(
  ...
  return (settings.pageColumns || 1) === 2 &&
    width >= 760
    ? 'always'
    : 'none';
}

function epubColumnCount(
  settings: ReaderSettings,
  width: number,
) {
  return epubSpread(settings, width) === 'always'
    ? 2
    : 1;
}

function epubLayoutKey(
  settings: ReaderSettings,
  width: number,
  height: number,
) {
  return [
    width,
    height,
    epubColumnCount(settings, width),
    settings.fontFamily,
    settings.fontSize,
    settings.fontWeight,
    settings.lineHeight,
...
function ImageZoomOverlay({
  ...
  return (
    ...
    >
      ...
      <div
        className="reader-image-scroll"
        onWheel={(event) => {
          if (!event.ctrlKey) return;

          event.preventDefault();
          change(event.deltaY < 0 ? 0.25 : -0.25);
        }}
      >
      ...
      <style>
        {`.reader-image-scroll img{${
          zoom === 0
            ? 'max-width:100%;max-height:100%;width:auto;height:auto'
            : `width:${zoom * 100}%;max-width:none;max-height:none;height:auto`
        }}`}
      </style>
    </div>
  );
...
function EpubSurface({
  ...
  onEpubLocations: Props['onEpubLocations'];
  navigationRef: React.MutableRefObject<NavigationApi | null>;
}) {
  ...
  const visualRef = useRef(onVisual);
  const cacheRef = useRef(onEpubLocations);

  settingsRef.current = settings;
  progressRef.current = onProgress;
  visualRef.current = onVisual;
  cacheRef.current = onEpubLocations;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const rendition =
        instance.renderTo(host.current, {
      ...
      /*
       * EPUB находится внутри iframe.
       * Поэтому мобильные события нужно
       * подключать непосредственно внутри
       * документа epub.js.
       */
      const wireGestures = (contents: any) => {
        ...
        if (
          ...
          documentRef.documentElement.dataset
            .magicReaderGestures === 'true'
        ) {
          return;
        }

        documentRef.documentElement.dataset.magicReaderGestures =
          'true';
        ...
        const handleTouchEnd = (
          ...
          if (
            !startTarget ||
            zoomOpenRef.current ||
            isInteractiveTarget(startTarget) ||
           ...
           * Он должен быть заметно сильнее
           * вертикального движения пальца.
           */
          if (
            Math.abs(deltaX) >= 50 &&
            Math.abs(deltaX) >
      ...
      const syncZoom = () => {
        ...
        window.setTimeout(() => {
          ...
          for (const contents of Array.isArray(
            rendered,
          )
            ? rendered
            : [rendered]) {
      ...
      navigationRef.current = {
        next: () =>
          enqueue(() => rendition.next()),

        prev: () =>
          enqueue(() => rendition.prev()),
          ...
          () =>
            void (async () => {
              ...
              try {
                ...
                measureRendition.hooks.content.register(
                  async (contents: any) => {
                    await contents.addStylesheet(
                      '/reader-fonts.css',
                    );
                    ...
                    await contents.document
                      ?.fonts?.ready;
                  },
                );

                const measureSpine = (
                  ((measureBook.spine as any)
                    ?.spineItems ||
                    []) as any[]
                ).filter(
                  (item) =>
                ...
                for (const item of measureSpine) {
                  ...
                  await measureRendition.display(
                    item.href,
                  );
                  ...
                  const location =
                    measureRendition.currentLocation();
        ...
        generationTimer =
          window.setTimeout(() => {
            const generate =
              async () => {
                try {
                  await instance.locations.generate(
                    1800,
                  );
  ...
  const palette =
    readerColors(settings);

  return (
    <div
      className="epub-surface-wrap"
      style={{
    ...
    >
      <div
        ref={host}
        className="epub-surface"
        aria-label="Текст книги"
      />
...
export function Reader({
  ...
  const handleStageTouchEnd = (
    ...
    if (
      !start ||
      event.changedTouches.length !== 1 ||
      isInteractiveTarget(start.target)
    ) {
      return;
    }
    ...
    <main className="reader-shell">
      ...
      <footer className="reader-bottombar">
        ...
        <div>
          <span>
            {visual.total > 0
              ? `${visual.label} ${visual.current} из ${visual.total}`
              : visual.label}
          </span>
